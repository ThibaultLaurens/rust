...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    fn warn_if_unreachable(&self, id: hir::HirId, span: Span, kind: &str) {
        if self.diverges.get() == Diverges::Always &&
            // If span arose from a desugaring of `if` then it is the access `_t`
            // in the temporary block `{ let _t = <diverges>; _t }` that we are about to lint on.
            // This gives suboptimal diagnostics and so we stop here and allow the block
            // of the `if`-expression to be linted instead.
    ...
    fn check_expr_meets_expectation_or_error(&self,
        ...
        self.demand_suptype(expr.span, expected_ty, ty);
        ty
    }
